...
const CustomInput = ({ type, placeholder, label, value, onChange }) => {
  ...
  return (
    <label htmlFor={label} className="relative block rounded-lg border w-full text-blue-600 dark:text-white focus-within:bg-transparent focus-within:border-white outline-none">
      ...
      <div className="flex box flex-row items-center w-full bg-blue-100 peer-focus:bg-blue-100 focus:bg-blue-100 rounded-lg" >
        <input
          ...
          id={label}
          className="peer border-none bg-inherit bg-blue-50 text-blue-600 w-full h-full focus:bg-blue-100 placeholder-transparent py-2.5 md:py-2 lg:py-2 px-2 xl:px-3.5 xl:py-3 text-sm md:text-sm lg:text-base focus:border-transparent focus:outline-none rounded-lg focus:ring-0 required:border-red-500"
        ...
        <span className="pointer-events-none absolute start-3.5 bg-transparent backdrop-opacity-20 backdrop-blur-sm peer-focus:bg-transparent top-0 -translate-y-1/2 p-0.5 text-sm text-blue-700 dark:peer-focus:top-2 font-bold transition-all peer-placeholder-shown:top-1/2 peer-placeholder-shown:text-sm peer-focus:top-0 peer-focus:text-xs">{label}</span>
      </div>
    </label>
  );
};
